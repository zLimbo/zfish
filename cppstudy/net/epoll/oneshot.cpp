...
void* worker(void* arg) {  //工作者线程(子线程)接收socket上的数据并重置事件
    ...
    while (1) {
        ...
        if (ret == 0) {                                   //关闭连接
            ...
            cout << buf;
            sleep(
                5);  //采用睡眠是为了在5s内若有新数据到来则该线程继续处理，否则线程退出
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    addfd(epollfd, listenfd,
          false);  //不能将监听端口listenfd设置为EPOLLONESHOT否则会丢失客户连接
    while (1) {
        int ret =
            epoll_wait(epollfd, events, MAX_EVENT_NUMBER, -1);  //等待事件发生
        if (ret < 0) {
        ...
        for (int i = 0; i < ret; i++) {
            ...
            if (sockfd == listenfd) {  //监听端口
                ...
                addfd(epollfd, connfd,
                      true);  //新的客户连接置为EPOLLONESHOT事件
            } else if (events[i].events &
                       EPOLLIN) {  //客户端有数据发送的事件发生
                pthread_t thread;
                ...
                pthread_create(
                    &thread, NULL, worker,
                    (void*)&fds_for_new_worker);  //调用工作者线程处理数据
            } else {
