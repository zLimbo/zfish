...
protected:
    ...
    Matrix(int rows, int cols)
        : rows_(rows), cols_(cols), linear_(new T[rows * cols]) {
...
public:
    ...
    RowMatrix(int rows, int cols)
        : Matrix<T>(rows, cols), data_(new T *[rows]) {
    ...
    void FillFrom(const std::vector<T> &source) override {
        size_t n = this->rows_ * this->cols_;
        if (source.size() != n) {
            throw Exception{ExceptionType::OUT_OF_RANGE,
                            "`source` is incorrect size"};
        }
...
public:
    ...
    static std::unique_ptr<RowMatrix<T>> Add(const RowMatrix<T> *matrixA,
        ...
        for (int i = 0; i < row; i++) {
            for (int j = 0; j < col; j++) {
                T ea = matrixA->GetElement(i, j);
                T eb = matrixB->GetElement(i, j);
                result->SetElement(i, j, ea + eb);
            }
        }
        return result;
    }
    ...
    static std::unique_ptr<RowMatrix<T>> Multiply(const RowMatrix<T> *matrixA,
        ...
        if (row == 0 || col == 0 || mid == 0) {
            return std::unique_ptr<RowMatrix<T>>(nullptr);
        }
        auto result = std::make_unique<RowMatrix<T>>(row, col);
        for (int i = 0; i < row; i++) {
            for (int j = 0; j < col; j++) {
                T e = matrixA->GetElement(i, 0) * matrixB->GetElement(0, j);
                for (int k = 1; k < mid; k++) {
                    e += matrixA->GetElement(i, k) * matrixB->GetElement(k, j);
                }
                result->SetElement(i, j, e);
            }
        }
        return result;
    }
    ...
    static std::unique_ptr<RowMatrix<T>> GEMM(const RowMatrix<T> *matrixA,
        ...
        if (auto result = Multiply(matrixA, matrixB); result != nullptr) {
            return Add(result.get(), matrixC);
        }
