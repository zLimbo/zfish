...
bool onLeft(Point2f &p, Point2f &q, Point2f &s) {
    int helenArea =
        p.x * q.y - p.y * q.x + q.x * s.y - s.x * q.y + s.x * p.y - p.x * s.y;
    return helenArea > 0;
}
...
void myConvexHull(vector<Point2f> &points, vector<int> &hull) {
    ...
    do {
        ...
        for (size_t k = 0; k < sz; ++k) {
            if (k != p && k != q &&
                (q == -1 || !onLeft(points[p], points[q], points[k])))
                q = k;
        }
        p = q;
    } while (p != ltl);
...
void myConvexHull2(vector<Point2f> &points, vector<int> &hull) {
    ...
    for (size_t k = 0; k < sz; ++k) {
        if (k != p && k != q) {
            double xk = points[k].x - points[p].x,
                   yk = points[k].y - points[p].y;
    ...
    for (auto it = auxMap.begin(); it != auxMap.end(); ++it) {
        if (onLeft(points[hull[hull.size() - 2]], points[hull[hull.size() - 1]],
                   points[it->second]))
